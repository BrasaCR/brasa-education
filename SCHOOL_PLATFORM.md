...
Ownership recovery requires the operator to repeat the exact school ID as confirmation and provide a bounded reason; an optional evidence reference is retained in the audit snapshot. Administrators can inspect the latest 250 audit events at `/audit` and download a bounded `brasa.school-export.v1` JSON package at `/export`. Exports contain school-owned settings, memberships, lessons, and up to 5,000 audit events, but never Identity records, invitation codes, bearer tokens, or service credentials.
