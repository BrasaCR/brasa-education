...
export async function changeSchoolLifecycle({ request, env, params }) {
  try {
    ...
    if (action === 'suspend') {
      ...
      const adminId = String(input.adminId || '').trim().toUpperCase(), reason=String(input.reason||'').trim(), evidenceRef=String(input.evidenceRef||'').trim(); if (!/^BRA-[A-Z0-9-]{5,40}$/.test(adminId) || reason.length<10 || reason.length>500 || input.confirm!==schoolId || evidenceRef.length>500) throw httpError(400, 'invalid_recovery');
      ...
      await env.DB.prepare('INSERT INTO school_audit_log (id,school_id,actor_brasa_id,action,resource_type,resource_id,snapshot_json,occurred_at) VALUES (?,?,?,?,?,?,?,?)').bind(crypto.randomUUID(), schoolId, 'system:platform-operator', action, 'school', schoolId, JSON.stringify({ newAdmin: adminId, reason, evidenceRef: evidenceRef||null, confirmedSchool:schoolId }), now).run();
