...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    const row = await env.DB
      .prepare("SELECT slug, name, type, canton, province, dre, region, level_id, tagline, doc, junta, cedula_juridica, sinpe_movil, sinpe_account, is_private FROM schools WHERE slug = ?")
      .bind(slug)
      .first();
