...
test('ownership recovery revokes old administrators and returns one-time invitation', async () => { const env = environment(), response = await changeSchoolLifecycle({ request: request({ action: 'recover', adminId: 'BRA-NEW-ADMIN-12345' }), env, params: { schoolId: 'school-a' } }), data = (await response.json()).data; assert.equal(response.status, 200); assert.equal(data.invitationCode, 'once'); assert.equal(data.adminId, 'BRA-NEW-ADMIN-12345'); });
